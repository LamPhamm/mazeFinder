...
def is_valid_move(maze,path):
    ...
    path_list=list(path)
    for move in path_list:
        #Check bound cases(Second bound case crashes program-not sure why, but works without it)
        if current_pos[0]==len(maze)-1:#End of the maze where the O reaches the X. Don't want the O to move down
            return False
        #elif current_pos[0]-1==-1:#Start of the maze at the top row at the current position. Don't want the O to move up
            #return False
...
maze=create_maze()
path=BFS(maze)
maze_solution=display_maze_solution(maze,path)
